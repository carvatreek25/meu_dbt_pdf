...
# Agora importamos o gerenciador de MySQL
from configs.tools.mysql import RDSMySQLManager

logging.basicConfig(level=logging.INFO)

class PDFTableExtractor:
    def __init__(self, file_name, configs):
        self.path = os.path.abspath(f"meu_dbt_pdf/files/pdf/{configs["name"].lower()}/{file_name}.pdf")
        self.csv_path = os.path.abspath(f"meu_dbt_pdf/files/csv/")
    ...
    def send_to_db(df, table_name):
        try:
            connection = RDSMySQLManager().alchemy()
...
if __name__ == "__main__":
    corretora = 'jornada'
    path = os.path.abspath(f"meu_dbt_pdf/files/pdf/{corretora}/")
